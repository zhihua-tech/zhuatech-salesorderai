...
CREATE DATABASE IF NOT EXISTS zhuatech_salesorderai DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE zhuatech_salesorderai;
...
CREATE TABLE sales_order_review (
  ...
  INDEX idx_salesorderai_status (review_status, created_at)
);

CREATE TABLE salesorderai_analysis_task (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_salesorderai_task (business_id, task_status)
);

CREATE TABLE salesorderai_audit_log (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  business_id BIGINT NOT NULL,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_salesorderai_audit (business_id, created_at)
);
